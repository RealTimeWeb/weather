...
; Build Client Store
(define CACHE-FILENAME "cache.json")
(cond [(not (file-exists? CACHE-FILENAME))
       (error (string-append "weather-service-offline: the local file " CACHE-FILENAME " could not be found."))])
(define CLIENT_STORE (read-json (open-input-file CACHE-FILENAME)))
...
(define (geocode/string address)
  (get->json "http://maps.googleapis.com/maps/api/geocode/json"
...
(define (get-report/string latitude longitude)
  (get->json "http://forecast.weather.gov/MapClick.php"
